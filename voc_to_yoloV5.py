      ...
      with open(txtfile, "w+" ,encoding='UTF-8') as out_file:
        ...
        for obj in root.iter('object'):#对文件进行遍历，找到armor和car两个object，对其进行处理和暂存
            ...
            if cls =='armor' : 
                ...
                if(armor_color=='red'):
                    armor_class[i]=(0,x,y)# 0,x,y    将装甲板中心坐标和代表颜色的编号暂存：红色
                    #print(armor_class[i])
                    i+=1#i最后的数值就是armor的个数
                if(armor_color=='blue'):
                    armor_class[i]=(1,x,y)# 1,x,y    将装甲板中心坐标和代表颜色的编号暂存：蓝色
                    #print(armor_class[i])
                    i+=1
                if(armor_color=='grey'):
                    armor_class[i]=(2,x,y)# 2,x,y    将装甲板中心坐标和代表颜色的编号暂存：灰色
                    #print(armor_class[i])
                    i+=1
                ...
                xmlbox = obj.find('bndbox')
                b = (float(xmlbox.find('xmin').text), float(xmlbox.find('xmax').text), float(xmlbox.find('ymin').text), float(xmlbox.find('ymax').text))
                car_class[j]=b# x_min,y_min,x_max,y_max 将车辆左上和右下角点坐标暂存至car_class
                j+=1
        #print(armor_class)
        ...
        for x_min,x_max,y_min,y_max in car_class:
            ...
            for color,x,y in armor_class:
                ...
                if(x==0 and y==0):
                    break
                if(x>=x_min and x<=x_max and y>=y_min and y<=y_max):#如果某个装甲板在当前的车ROI里，即找到了当前车的颜色
                    #print('\nfound proper box\n')
                    bb=convert((w,h),(x_min,x_max,y_min,y_max))#这时再将角点坐标转换为中心坐标和长宽
                    out_file.write(str(color) + " " + " ".join([str(a) for a in bb]) + '\n')#将类别名、中心坐标、长、宽输出至文件，保存为yolov5格式
                    break
